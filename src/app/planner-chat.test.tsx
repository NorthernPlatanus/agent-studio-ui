...
describe("planner chat", () => {
  ...
  it("sends attached files with the create request, not after it", async () => {
    ...
    await user.type(await screen.findByLabelText(/opening message/i), "fix the camera");
    ...
    await user.upload(
      ...
      new File(["the camera jitters on respawn"], "triage notes.md", { type: "text/markdown" }),
    );
    ...
    expect(bodies[0]?.uploads).toEqual([
      { name: "triage notes.md", text: "the camera jitters on respawn" },
    ]);
  });
